...
class ECDSA extends AsymmetricKey
{
    ...
    public function sign($message, $format = 'ASN1')
    {
        ...
        if ($this->curve instanceof TwistedEdwardsCurve) {
            ...
            $hash = new Hash($this->curve::HASH);

            $secret = substr($hash->hash($this->dA->secret), $this->curve::SIZE);
            ...
            $R = $this->curve->multiplyPoint($this->curve->getBasePoint(), $this->curve->convertInteger($r));
            $R = $this->curve->encodePoint($R);
            ...
            $S = str_pad(strrev($S->toBytes()), $this->curve::SIZE, "\0");
            return $R . $S;
        }
    ...
    public function verify($message, $signature, $format = 'ASN1')
    {
        ...
        if ($this->curve instanceof TwistedEdwardsCurve) {
            ...
            $R = substr($signature, 0, $this->curve::SIZE);
            $S = substr($signature, $this->curve::SIZE);

            try {
                $R = PKCS1::extractPoint($R, $this->curve);
            ...
            $dom2 = !isset($this->context) ? '' :
                'SigEd25519 no Ed25519 collisions' . chr(0) . chr(strlen($this->context)) . $this->context;

            $hash = new Hash($this->curve::HASH);
            $k = $hash->hash($dom2 . substr($signature, 0, $this->curve::SIZE) . $A . $message);
            $k = strrev($k);
            $k = new BigInteger($k, 256);
            list(, $k) = $k->divide($order);

            $qa = $this->curve->convertToInternal($this->QA);

            $lhs = $this->curve->multiplyPoint($this->curve->getBasePoint(), $this->curve->convertInteger($S));
            $rhs = $this->curve->multiplyPoint($qa, $this->curve->convertInteger($k));
            $rhs = $this->curve->addPoint($rhs, $R);
            $rhs = $this->curve->convertToAffine($rhs);
