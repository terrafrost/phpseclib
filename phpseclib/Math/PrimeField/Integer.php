...
class Integer extends Base
{
    ...
    {
        $temp = new static($this->instanceID);
        $temp->value = $this->value->powMod($x, static::$modulo);
    ...
    public function squareRoot()
    {
        ...
        for ($z = $one; !$z->equals(static::$modulo); $z = $z->add($one)) {
            $temp = $z->powMod($pow, static::$modulo);
            if ($temp->equals($p_1)) {
                break;
            }
        }
        ...
        $m = new BigInteger($s);
        $c = $z->powMod($q, static::$modulo);
        $t = $this->value->powMod($q, static::$modulo);
        list($temp) = $q->add($one)->divide($two);
        $r = $this->value->powMod($temp, static::$modulo);

        while (!$t->equals($one)) {
            $i = clone $one;

            while (!$t->powMod($two->pow($i), static::$modulo)->equals($one)) {
                $i = $i->add($one);
            }
            ...
            $b = $c->powMod($two->pow($m->subtract($i)->subtract($one)), static::$modulo);
            $m = $i;
